...
x = np.outer(np.ones(np.size(v)),2*np.cos(u))
y = np.outer(np.ones(np.size(v)),2*np.sin(u))
z = np.outer(np.ones(np.size(v)),8-2*np.sin(u))
...
ax.view_init(elev=20.0, azim=25.0)
